import 'package:sqflite/sqflite.dart';
import 'package:path/path.dart';
import './todo_model.dart';

class DatabaseConnect {
  Database? _database; //_はプライベート変数

  //create a getter and open a connection to database
  Future<Database> get database async {
    //this is the location of our database in devices.
    final dbpath = await getDatabasesPath();
    //this is the name of our database.
    const dbname = 'todo.db';
    //this joins the dbpath and dbname and creates a full path for database.
    final path = join(dbpath, dbname); //joinは文字列をつなげる
    //open the connection
    _database = await openDatabase(path,
        version: 1, onCreate: _createDB); //onCreateはデータベースがない場合に呼び出される
    //we will create the _createDB function separately.
    return _database!;
  }

  //_the _create db function
  //this creates Tables in our database.
  Future<void> _createDB(Database db, int version) async {
    //make sure the columns we create in our table match the todo_model field.
    await db.execute('''
      CREATE TABLE todo(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        title TEXT,
        creationDate TEXT,
        startTime TEXT,
        elapsedTime INTEGER,
        isChecked INTEGER,
        isHabit INTEGER
      )
    ''');
  }

  //function to add data into our database
  Future<void> insertTodo(Todo todo) async {
    //get the connection to database
    final db = await database;
    //insert the todo
    await db.insert(
      'todo', // the name of our table
      todo.toMap(), //the functuion we created in our todo_model
      conflictAlgorithm:
          ConflictAlgorithm.replace, //this will replace the duplicate entries.
    );
  }

  //function to delete a todo from our database.
  Future<void> deleteTodo(Todo todo) async {
    final db = await database;
    //delete the todo from database based on its id.
    await db.delete(
      'todo',
      where: 'id == ?', //this condition will check for id intodo list
      whereArgs: [todo.id],
    );
  }

  //function to fetch all the todo data from our database
  Future<List<Todo>> getTodo(String selectedDay) async {
    final db = await database;
    //query the database and save the todo as list of maps
    List<Map<String, dynamic>> items = await db.query(
      'todo',
      where: "creationDate=?",
      whereArgs: [selectedDay],
      orderBy: 'id ASC', //this will order the list by id in descending order
    ); //so the latest todo will be displayed on top.

    //now convert the items from list of maps to list of todo
    return List.generate(
      items.length,
      (i) => Todo(
        id: items[i]['id'],
        title: items[i]['title'],
        creationDate: DateTime.parse(items[i]['creationDate']),
        startTime: DateTime.parse(items[i]['startTime']),
        elapsedTime: items[i]['elapsedTime'],
        //this is in text format right now. lets conver it to dateTime format.
        isChecked: items[i]['isChecked'] == 1 ? true : false,
        isHabit: items[i]['isHabit'] == 1 ? true : false,
        //this will convert the Integer to boolean 1 = true, 0 = false
      ),
    );
  }
}
